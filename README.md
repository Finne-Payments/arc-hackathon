...
## Where models run (the agentic layer)
...
- **Where it runs:** on Finné-controlled machines with open weights. Inference is a self-hosted OpenAI-compatible HTTP endpoint — **vLLM on an AWS GPU** (production) or **Ollama** on the build laptop (dev). **No case content is sent to an external model API.** The backend boot-fails if any external model vendor key (`OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `AWS_BEDROCK_*`, …) is present (FIN-102). See [`docs/models.md`](docs/models.md) for the pinned models and swap rules.
- **Served model:** `Qwen/Qwen2.5-3B-Instruct` (~3B params, fits a single L4/A10G). The weights are **baked into the Docker image at build time** (`model/Dockerfile`), so a fresh GPU box doesn't re-download the model on first request — `docker run` goes straight to the VRAM load. Build with `./scripts/model-bake.sh`; see [`model/README.md`](model/README.md).
...
- **What it may never do:** render a verdict, mark an outcome correct, hold keys, sign, or be required for the loop. Every model call has a 5-second hard timeout and a defined degrade path (P8); CI proves the full demo loop passes with the model permanently unplugged (`FIN-105`). The agent saves time — it is never the thing the case depends on.
...
│       ├── v1/                   canonical v1 API layer (router, models, services, chain verifier)
│       ├── agent/                frame assembly, turning questions, narrative, model client (FIN-101)
...
│   └── src/screens/             Legacy: Ledger, NewPayout, Receipt, CaseRoom, Decision, Disputes,
│                                RecipientHome, Platform, Login
│       └── v1/                   Canonical v1: CaseRoom, Correction, Dashboard, Decision, FramePanel, V1App
...
├── openapi/                     finne-v1.yaml — the canonical v1 API contract (36 operations)
├── scripts/                     dev.sh, demo.sh, deploy-arc.sh, model-bake.sh
├── docs/                        adr/, scope/, security/, models.md, architecture.md, mvp-progress.md
├── .github/workflows/           ci.yml (typecheck · test · build · secret-scan · abi-quarantine)
│                                deploy.yml (CDK deploy to AWS on push to main/develop)
├── docker-compose.yml           backend + web + model profiles (gpu/dev)
└── .env.example                 deployment env (placeholders only — real secrets stay local)
```

## Tech stack

- **Contracts** — Solidity ^0.8.24, Foundry (forge), OpenZeppelin (AccessControl, EIP-712, IERC20). The audited set is Circle's; Finné's additions harden the drain + reentrancy paths and add the dispute lifecycle (see `contracts/refund-protocol/README.md`).
...
The infra lives in `infra/cdk/` — two stacks deployed via CDK:

- **FinneStack** — VPC, ECS Fargate (backend + web), ECR, S3 evidence bucket, SQS + DLQ, KMS, Secrets Manager, ALB.
- **FinneModelStack** — GPU EC2 instance (g5.xlarge) running the pre-baked vLLM image, reachable from the backend over private DNS.

The deploy pipeline (`.github/workflows/deploy.yml`) auto-deploys on push to `main`/`develop`: it runs the CI gates, builds and pushes images to ECR, syncs secrets to AWS Secrets Manager, then runs `cdk deploy --all`. For the one-time AWS bootstrap (IAM user, CDK bootstrap, GitHub secrets), see [`infra/SECRETS.md`](infra/SECRETS.md).
...
cd contracts/refund-protocol && forge test    # contract tests
```
...
- **Backend**: 159 tests across 13 files — agent guardrails, frame symmetry, checks engine, v1 integration + security, legacy routes, unplugged-mode guarantee (FIN-105).
...
3. **Secret scan** — gitleaks (open-source binary, no paid license) scans the full git history for leaked credentials. Template files (`.env.example`, `infra/SECRETS.md`) are allowlisted — they contain only placeholders.
4. **ABI quarantine** (RP-03) — ensures the v1 production routes never import the legacy `RefundProtocol` ABI or reference `refundByArbiter`.

## External dependencies (credentials to provide)
...
|---|---|---|
| **Arc testnet wallets** | `ARC_USDC_ADDRESS`, `CASE_REGISTRY_ADDRESS`, role wallets | Real USDC transfers, contract deployment, chain verification |
...
| **IdP/OIDC** | `IDP_ISSUER`, `IDP_CLIENT_ID`, `IDP_CLIENT_SECRET` | Cognito/OIDC for operations + reviewer auth |
